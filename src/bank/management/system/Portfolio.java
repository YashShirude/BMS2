...
public class Portfolio extends JFrame implements ActionListener{
    ...
    String accType;
    int balance,totalFd,totalRd,totalPrincipal;
    ...
    Portfolio(){
        ...
        SqlQuery query18 = new SqlQuery();
        query18.fetchDepositDetails(SqlQuery.accNo);
        totalPrincipal = SqlQuery.totalPrincipal;
        totalFd = SqlQuery.totalFd;
        totalRd = SqlQuery.totalRd;
...
//        }
        ...
        JLabel h2 = new JLabel("Deposits:       Rs. " + totalPrincipal);
        h2.setBounds(30,300,600,30);
        h2.setForeground(Color.black);
        h2.setFont(new Font("System", Font.BOLD, 30));
        add(h2);

        JLabel h2a = new JLabel("(" + totalFd + "Fixed Deposit & " + totalRd + " Recurring Deposit)");
    ...
    public void actionPerformed(ActionEvent ae){
        ...
        }else if(ae.getSource() == settleDraftButton){
            if(balance >= overDraftUsed){
                // update financial Details
                // transactional details
                CurrentAcc c = new CurrentAcc();
                balance -= overDraftUsed;
                SqlQuery query19 = new SqlQuery();
                query19.updateFinancialDetails(accountNo, String.valueOf(balance));
                query19.insertTransactionDetails(accountNo,"Overdraft Settled",String.valueOf(overDraftUsed),"Debited",String.valueOf(balance));
                query19.updateTransaction(accountNo,remTransactions,c.totalOverdraft());
                setVisible(false);
                new HomePage().setVisible(true);
            }else{
